...
const CategoryPage = props => {
    ...
    const { productpath, /*index, first, last, pageCount*/ } = pageContext
    //const previousUrl = index - 1 === 1 ? '/c/alcool-gel-hidratante-antisseptico/' : `/c/alcool-gel-hidratante-antisseptico/${(index - 1).toString()}`
   // const nextUrl = `/c/alcool-gel-hidratante-antisseptico/${(index + 1).toString()}`
...
export const query = graphql`
    query($categoryregex: String!, $skip: Int!) {
        allShopifyProduct(
            limit: 36,
            skip: $skip,
            filter: {metafields: {elemMatch: {value: {regex: $categoryregex}, key: {eq: "categoryurl"}}}},
            sort: {
